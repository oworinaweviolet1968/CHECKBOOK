...
public class ReceiptPrinterService {
    ...
    // 0x01 = double height (normal width, allows up to 20 chars for amount)
    private static final byte[] GS_DOUBLE_HEIGHT = { 0x1D, 0x21, 0x01 };
    ...
    private static final long PRINT_COOLDOWN_MS = 3000; // 3 seconds rate limit to prevent duplicate prints
    ...
    private static synchronized void doPrint(ObservableList<SaleItem> items, String customer, String totalAmount,
            String date) {
        ...
        if (printer == null) {
            System.err.println(
                    "❌ Critical: No printers detected at all. Please add your printer in 'Settings > Printers'.");
            return;
        }
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            // ═══════════════════════════════════════════
            // 2. HEADER — "CHECKBOOK APP" in double size (matching mobile
            // PosTextSize.size2)
            // ═══════════════════════════════════════════
            ...
            // ═══════════════════════════════════════════
            // Mobile layout: PosColumn(Item, w=6), PosColumn(Qty, w=2), PosColumn(Total,
            // w=4)
            ...
            String headerLine = padRight(headerItem, 16) + padRight(headerQty, 5)
                    + padLeft(headerTotal, LINE_WIDTH - 16 - 5);
            ...
            // ═══════════════════════════════════════════
            // 8. TOTAL SECTION — matching mobile: double-height bold, fits 10+ digits
            // ═══════════════════════════════════════════
            baos.write("--------------------------------\n".getBytes());
            baos.write(ESC_BOLD_ON);
            baos.write(GS_DOUBLE_HEIGHT);
            long total = parseAmount(totalAmount);
            String totalLabel = "TOTAL AMOUNT";
            String totalValStr = df.format(total);

            // Using double-height (normal width = 32 chars line width)
            // Leaves 20 chars for amount (fits up to 15-digit numbers with commas!)
            int totalPad = LINE_WIDTH - totalLabel.length() - totalValStr.length();
            String totalLine = totalLabel + (totalPad > 0 ? " ".repeat(totalPad) : " ") + totalValStr;
            ...
            baos.write("Great having you! See you soon!\n".getBytes());
            baos.write("Powered by CHECKBOOK mobile\n".getBytes());
