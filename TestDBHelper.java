public class TestDBHelper {
    public static void main(String[] args) {
        String[] bulkUnits = {"box*20", "kg", "pcs", "sack"};
        String[] sizes = {"1kg", "50kg"};
        double[] totals = {200.0, 196.0};

        for (String bulkUnit : bulkUnits) {
            for (String size : sizes) {
                for (double totalBase : totals) {
                    System.out.printf("totalBase=%f, size=%s, bulkUnit=%s -> %s\n",
                        totalBase, size, bulkUnit, formatStockForDisplay(totalBase, size, bulkUnit));
                }
            }
        }
    }
